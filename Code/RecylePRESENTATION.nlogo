...
;  random-seed 102
  ...
  ;let trash n-of trashbins patches
  ask trash [set pcolor gray]
    let recycling n-of recyclingbins patches with [ pxcor < -3 and pycor > 4 ]
  ask recycling [set pcolor green]]
  if mode = "baseline"
   [ random-seed 202
     if trashbins = 1
...
to go
;  if ticks > 0 and ticks mod 3000 = 0 [stop]
  ask turtles [
    if wealth >= 0 [move]
...
to disposewaste
  ...
  set normalizedIdeology ([ideology] of self - min [ideology] of turtles)/ (max [ideology] of turtles - min [ideology] of turtles)
  ifelse ((mean ([normalizedIdeology] of turtles-on neighborhood)) * (number-of-turtles * 0.1)) + (normalizedIdeology) >= 1.3
...
;    set ideology ideology + ((mean ([ideology] of turtles-on neighborhood)) * (number-of-turtles * 0.1))
  ...
  [let closest_bin min-one-of patches with [pcolor != 0]  in-radius vision [distance myself]
    ...
    if EconomicIncentive? [set wealth wealth + EconomicIncentive]]
      ...
      set ideology ideology - (( mean ([ideology] of turtles-on neighborhood)) * (number-of-turtles * 0.1))]
    set normalizedIdeology ([ideology] of self - min [ideology] of turtles)/ (max [ideology] of turtles - min [ideology] of turtles)
    set normalizedIdeology ideology + (mean ([normalizedIdeology] of turtles-on neighborhood))
  ]
end
...
788
169
Mean Normalized Turtle Ideology
NIL
ideology
0.0
1.0
...
788
346
Average Wealth Distribution  Brown Agents
NIL
NIL
0.0
10.0
...
mode
mode
"changing location" "baseline" "scenario 2"
0
...
489
345
Average Wealth Distribution Green Agents
NIL
NIL
0.0
10.0
...
528
Wealth Green Agents
mean [wealth] of turtles with [color = green]
2
1
11
